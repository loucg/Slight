...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="lamp/getLampList" method="post" name="Form" id="Form">
						...
						<table style="margin-top:5px;">
							<tr>
								...
								<c:if test="${QX.cha == 1 }">
								<td style="vertical-align:top;padding-left:2px;"><a class="btn btn-light btn-xs" onclick="search();"  title="<%=search %>"><i id="nav-search-icon" class="ace-icon fa fa-search bigger-110 nav-search-icon blue"></i></a></td>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center"><%=strandard %>/<%=name %></th>
							...
							<!-- 开始循环 -->
							<c:choose>
								<c:when test="${not empty lampList}">
									<c:if test="${QX.cha == 1 }">
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="<%=add%>";
			 diag.URL = '<%=basePath%>lamp/goLampCreate';
			 diag.Width = 650;
			 diag.Height = 224;
		...
		function fromExcel(){
			 ...
			 diag.Drag=true;
			 diag.Title ="<%=importt%>";
			 diag.URL = '<%=basePath%>lamp/goUploadExcel';
			 diag.Width = 300;
			 diag.Height = 150;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
