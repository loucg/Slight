...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="config/getSimList" method="post" name="Form" id="Form">
									...
									<c:forEach items="${simList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center' style="width: 30px;">${vs.index+1}</td>
													...
													<td>${ fn:substring(var.mobile,0,50)}</td>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>config/goSimCreate';
			 diag.Width = 650;
			 diag.Height = 279;
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>config/goSimEdit?id='+Id;
			 diag.Width = 650;
			 diag.Height = 279;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>simcard/excel.do';
		}
		</script>
