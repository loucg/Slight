				...
				<div class="page-content">
					<div class="row">
						...
						<table style="margin-top:5px;">
							<tr>
								...
								<td>
									<div class="nav-search">
										<span class="input-icon">
											<input type="text" class="nav-search-input" id="nav-search-input" autocomplete="off" name="code" value="${pd.code }"/>
										</span>
									</div>
								</td>
								<td>&nbsp;&nbsp;位置：</td>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty vofaultList}">
									<c:if test="${QX.cha == 1 }">
									<c:forEach items="${vofaultList}" var="vofault" varStatus="vs">
										<tr>
											<td class='center' style="width: 30px;">${vs.index+1}</td>
											<td class='center'>${vofault.code}</td>
											<td class='center'>${vofault.name}</td>
											<td class='center'>${vofault.location}</td>
											<td class='center'>${vofault.lamp_pole_num}</td>
									        <td class='center'>${vofault.tname}</td>
									        <td class='center'>${vofault.tdate}</td>
									        <td class='center'>${vofault.comment}</td>
										</tr>
									</c:forEach>
									</c:if>
									<c:if test="${QX.cha == 0 }">
